...
{
    public partial class PolishForm : Form
    {
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    new SqlParameter("@Day", polish), // จำนวนวันที่ดึงจากฐานข้อมูล
                    new SqlParameter("@Inv", txtInv.Text), // หมายเลข IV ที่ผู้ใช้กรอก
                    new SqlParameter("@minQty", minQty) // จำนวนขั้นต่ำที่ดึงจากฐานข้อมูล
                };
                DataTable dt = db.ExecuteQuery("sp_Find_Inv_Polish", param, true); // เรียก Stored Procedure และเก็บผลลัพธ์
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    report.ShowReport("PolishReport2.rpt", dt); // ใช้ ReportGenerator แสดงรายงาน
                }
                else
                {
        ...
        private void GetData()
        {
            ...
            string query = "select top 1 Prolish, minQty from SetDateAndSilver order by cDate desc;";
            ...
            if (dt.Rows.Count > 0) // ตรวจสอบว่าพบข้อมูลหรือไม่
            {
                ...
                polish = Convert.ToInt32(dt.Rows[0]["Prolish"]);
